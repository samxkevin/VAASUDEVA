...
//We use showbase to show the base for integral types. (i.e. dec no changes, hex shows 0x, oct has 0
//in front)
//we can std::uppercase and std::lowercase for show base to be in upper or lower cases
//we use std::scientific and std::fixed for literals to be shown in... you know. By default allt he numbers 
//are shown in the user format but by fixed we can let it to be shown by precision or by scientific in
//the scientific format. and double c{1.34e-10}--- evaluates to 0.000000. To set the floating points to the
//default value we use cout.unset(std::ios::scientific|std::ios::fixed) we can set precision of a floating
//point variable. The showpoint decimals after the literals needed. For example if we execute std::showpoint
//after the given code and output all the variables we then we get all the decimals upto 7 literals. 
/* double d {34.1};
double e {101.99};
double f {12.0};
int g {45}; *///no decimal point here so no decimals after the literals
//Refer these images...
//"C:\Users\anumu\Pictures\Screenshots\CPP.png"
//"C:\Users\anumu\Pictures\Screenshots\CPP2.png"
//Refer this link: https://en.cppreference.com/w/cpp/io/manip
//endl and "/n" work very differently under the hood
//By std::flush the data is sent directly to output and it doesn't pass from any intermediary buffer
//Implementation of setw()
/* #include<bits/stdc++.h>
#include<iomanip>
using namespace std;
int main(){
    cout<<setw(20)<<"Krishna:"<<endl;
    cout<<setw(20)<<" The Supreme Personality Of GodHead"<<endl;
    cout<<endl;
    cout<<setw(17)<<right<<"Affliation:"<<setw(5)<<left<<"Bhagawan"<<endl;
    cout<<setw(17)<<right<<"Abode:"<<setw(5)<<left<<"Goloka"<<endl;
    cout<<setw(17)<<right<<"Mantra:"<<setw(5)<<left<<"Hare Krishna"<<endl;
    cout<<setw(17)<<right<<" "<<setw(5)<<left<<"Maha Mantra"<<endl;
    cout<<setw(17)<<right<<"Weapon:"<<setw(5)<<left<<"Sudarshana"<<endl;
    cout<<setw(17)<<right<<" "<<setw(5)<<left<<"Chakra"<<endl;
    cout<<setw(17)<<right<<"Scriptures:"<<setw(5)<<left<<"Bhagavad"<<endl;
    cout<<setw(17)<<right<<" "<<setw(5)<<left<<"Gita"<<endl;
} */
//we can set the width to any variable and put the variable in the parenthesis of setw() this is more
//efficient since we don't have to delete every arguement given in the parenthesis if there is any 
//requirement, use std::internal for negitive numbers and build a code...
//if we set more precision in setprecision() than the precision of the data_type then the actual values are
//followed by Garbage valuesafter the real value...
//LIMITS
//The limits Library in CPP i.e., included using #include<limits> line of code which handles the limits of 
//different types of integer values. For example the lowest(minimum -ve value) for floating point values is
//-3.40282e+38, similarly min(lowest +ve value) is I . 17549+38 and maximum is 3.40282e+38. For integral 
//values(Signed integers) there is no lowest(terminology is the same) value, min is -32768 and max is 32767
//if unsigned(short) then min is 0 and max is 65535. Check this code:
#include<iostream>
#include<iomanip>