...
def playgame():
    ...
    while True:
        ...
        if "yes" in order:
            ...
            break
def wikipediasummary(order,lines):
    try:
        context=readablilityEnhancer(wikipedia.summary(order,sentences=lines))
    ...
    return context
def other(order,lines):
    context=wikipediasummary(order,lines)
    assess={'question':order,'context':context}
    try:
        response=nlp(assess)
...
def maincode():
    ...
    while True:
         ...
         if order=="none":
            ...
            speak(wikipediasummary(order,3))
            ...
            playmusic(songname)       
            print("DAS is now in Stand-By mode... Call him out whenever you need assistance.")  
...
You can press Ctrl+C(In the terminal) to stop listening...""")
    speak("hey there! das in stand-by mode here. just call out the wake phrase for activativating the operational mode.")
    try:
        while True:
